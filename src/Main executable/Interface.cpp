...
extern void CaptureMouseOnce(SDL_Window* gWindow);
extern void ResetMouseCapture();
...
int processMainMenu()
{
	...
	if (gWindow) {
		printf("processMainMenu: Restoring windowed mode\n");
		window_mode = true;
		...
		// Восстанавливаем размер окна из сохраненных значений
		RealLx = 1024;
		RealLy = 768;
		SDL_SetWindowSize(gWindow, RealLx, RealLy);
		SDL_SetWindowPosition(gWindow, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED);

		// Сбрасываем смещения для оконного режима
		menu_x_off = 0;
		menu_y_off = 0;
		menu_hint_x = 18;
		menu_hint_y = 701;
		...
		printf("processMainMenu: gWindow is null, attempting to recreate\n");
		CreateDDObjects(hwnd ? hwnd : GetActiveWindow());
		// Устанавливаем размер по умолчанию, если окно было пересоздано
		RealLx = 1024;
		RealLy = 768;
		if (gWindow) {
			SDL_SetWindowSize(gWindow, RealLx, RealLy);
		...
		printf("processMainMenu: Window recreated with default size %dx%d\n", RealLx, RealLy);
	}
	if (window_mode)
...
KRT:
	...
	{//Enter key
		...
		{//IMPORTANT: Handling cheats
			...
			if (!strcmp(ChatString, "house"))
			{
				GetHouse0();
			}
			if (!strcmp(ChatString, "nocost"))
			{
				Getnocost();
			}
			if (!strcmp(ChatString, "peace"))
			{
				Getpeace();
			}
