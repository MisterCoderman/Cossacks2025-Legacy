...
{
	// Проверка на nullptr и видимость
	if (!SD || !SD->Visible) {
		return false;
	}

	// Используем static_cast вместо dynamic_cast, если SimpleDialog не полиморфный
	ComplexBox* LB = static_cast<ComplexBox*>(SD);
	if (!LB) {
		return false; // Выходим, если приведение не удалось
	}

	// Обновление MouseOver
	if (!LB->MouseOver) {
		LB->M_OvrItem = -1;
	}
	...
	if (LB->VS) {
		...
		if (mxp < 0) {
			...
			LB->VS->SMaxPos = mxp;
			if (LB->VS->SPos > LB->VS->SMaxPos) {
				LB->VS->SPos = LB->VS->SMaxPos;
			}
			LB->VS->Visible = 1;
			LB->VS->Enabled = 1;
		}
		...
		LB->FirstVisItem = 0; // Если нет полосы прокрутки, начинаем с 0
	}

	// Проверка корректности параметров списка
	if (LB->NOnScr <= 0 || LB->N < 0 || !LB->DRAWPROC) {
		return true; // Выходим, если данные некорректны или нет функции отрисовки
	}

	// Цикл по видимым элементам
	for (int i = 0; i < LB->NOnScr; i++) {
		...
		// Проверка на валидность GP_Index и других параметров перед отрисовкой
		if (LB->GP_Index < -1 || LB->StSprite < 0) {
			continue; // Пропускаем, если параметры графики некорректны
		}

		// Отрисовка активного или выбранного элемента
		try {
			if (LB->M_OvrItem == cp) {
				if (LB->GP_Index != -1) {
					GPS.ShowGP(xx, yy, LB->GP_Index, LB->StSprite + 3 + (cp % 3), 0);
				}
				state = 2;
			}
			else if (LB->CurrentItem == cp) {
				if (LB->GP_Index != -1) {
					GPS.ShowGP(xx, yy, LB->GP_Index, LB->StSprite + (cp % 3), 0);
				}
				state = 1;
			}

			// Проверка функции отрисовки
			if (LB->DRAWPROC) {
				LB->DRAWPROC(xx, yy, LB->x1 - LB->x, LB->OneLy, cp, state, LB->param);
			}
		}
		catch (...) {
			// Логирование ошибки (замените на вашу систему логирования)
			// Например: LogError("Exception in ComplexBox_OnDraw at index %d", cp);
			continue; // Пропускаем элемент при любой ошибке
		}
	}
...
__declspec(dllexport) void CBar(int x0, int y0, int Lx0, int Ly0, byte c) {
	...
	// Проверка размеров
	if (Lx0 <= 0 || Ly0 <= 0) return;

	// Проверка буфера экрана
	if (!ScreenPtr) return;

	// Корректировка координат и размеров с учётом границ экрана
	int x = x0;
	int y = y0;
	int Lx = Lx0;
	int Ly = Ly0;

	// Обрезка по левой и верхней границе
	if (x < 0) {
		Lx += x; // Уменьшаем ширину
		x = 0;
	}
	if (y < 0) {
		Ly += y; // Уменьшаем высоту
		y = 0;
	}

	// Обрезка по правой и нижней границе
	if (x + Lx > RealLx) {
		Lx = RealLx - x;
	}
	if (y + Ly > RealLy) {
	...
	// Если после обрезки размеры некорректны, выходим
	if (Lx <= 0 || Ly <= 0) return;

	// Вычисляем начальный адрес в буфере экрана
	byte* dest = (byte*)ScreenPtr + x + y * SCRSizeX;

	// Заполняем прямоугольник построчно
	for (int i = 0; i < Ly; i++) {
		memset(dest, c, Lx); // Заполняем строку цветом c
		dest += SCRSizeX;    // Переходим к следующей строке
	}
}
...
bool BPXView_OnDraw(SimpleDialog* SD) {
	...
	// Приведение к BPXView
	BPXView* BV = (BPXView*)SD;
	if (!BV) return false;

	// Проверка буферов экрана и данных
	if (!ScreenPtr || !BV->Ptr) return false;

	// Фикс некорректных размеров сетки
	if (BV->Nx <= 0) BV->Nx = 1;
	if (BV->Ny <= 0) BV->Ny = 1;
	if (BV->OneLx <= 0) BV->OneLx = 1;
	if (BV->OneLy <= 0) BV->OneLy = 1;
	if (BV->RealNy <= 0) BV->RealNy = BV->Ny;

	// Обновление позиции скролла с защитой
	if (BV->VSC) {
		BV->PosY = BV->VSC->SPos;
	}
	else {
	...
	// Ограничение PosY в безопасных пределах
	if (BV->RealNy < BV->Ny) BV->RealNy = BV->Ny;
	if (BV->PosY > BV->RealNy - BV->Ny) BV->PosY = BV->RealNy - BV->Ny;
	if (BV->PosY < 0) BV->PosY = 0;

	// Фикс координат, чтобы они не выходили за экран
	int x = BV->x;
	int y = BV->y;
	if (x < 0) x = 0;
	if (y < 0) y = 0;
	if (x >= RealLx) x = RealLx - 1;
	if (y >= RealLy) y = RealLy - 1;

	// Вычисление размеров видимой области
	int totalLx = BV->Nx * BV->OneLx;
	int totalLy = BV->Ny * BV->OneLy;

	// Обрезка размеров, чтобы не выходить за экран
	if (x + totalLx > RealLx) totalLx = RealLx - x;
	if (y + totalLy > RealLy) totalLy = RealLy - y;
	if (totalLx <= 0 || totalLy <= 0) return false;

	// Вычисление количества элементов после обрезки
	int visibleNx = totalLx / BV->OneLx;
	int visibleNy = totalLy / BV->OneLy;
	if (visibleNx <= 0) visibleNx = 1;
	if (visibleNy <= 0) visibleNy = 1;

	// Отрисовка сетки
	byte* srcBase = (byte*)BV->Ptr + BV->PosY * BV->OneLx * BV->Nx * BV->OneLy;
	byte* destBase = (byte*)ScreenPtr + x + y * SCRSizeX;

	if (BV->DECLX) {
		// Отрисовка с промежутками между элементами
		for (int iy = 0; iy < visibleNy; iy++) {
			for (int ix = 0; ix < visibleNx; ix++) {
				int elemLx = BV->OneLx;
				int elemLy = BV->OneLy;

				// Обрезка размеров элемента
				int elemX = x + ix * BV->OneLx;
				int elemY = y + iy * BV->OneLy;
				if (elemX + elemLx > RealLx) elemLx = RealLx - elemX;
				if (elemY + elemLy > RealLy) elemLy = RealLy - elemY;
				if (elemLx <= 0 || elemLy <= 0) continue;

				// Вычисление адресов с учётом промежутков
				byte* src = (byte*)BV->Ptr + (BV->PosY + iy) * (BV->OneLx + BV->DECLX) * BV->Nx * (BV->OneLy + BV->DECLX) +
					ix * (BV->OneLx + BV->DECLX);
				byte* dest = (byte*)ScreenPtr + elemX + elemY * SCRSizeX;

				// Копирование данных построчно
				for (int dy = 0; dy < elemLy; dy++) {
					memcpy(dest, src, elemLx);
					src += (BV->OneLx + BV->DECLX) * BV->Nx;
					dest += SCRSizeX;
				}
			}
		}
		...
		// Отрисовка как единого блока
		int lineBytes = totalLx;
		byte* src = srcBase;
		byte* dest = destBase;
		for (int dy = 0; dy < totalLy; dy++) {
	...
	// Отрисовка выделенных элементов и текста
	for (int ix = 0; ix < visibleNx; ix++) {
		for (int iy = 0; iy < visibleNy; iy++) {
			int index = ix + (iy + BV->PosY) * BV->Nx;

			// Проверка выхода за границы массива Choosed
			if (index < 0 || index >= BV->Nx * BV->RealNy) continue;

			if (BV->Choosed && BV->Choosed[index]) {
				int elemX = x + ix * BV->OneLx;
				int elemY = y + iy * BV->OneLy;
				int drawLx = BV->OneLx - 1;
				int drawLy = BV->OneLy - 1;

				// Обрезка размеров рамки
				if (elemX + drawLx >= RealLx) drawLx = RealLx - elemX - 1;
				if (elemY + drawLy >= RealLy) drawLy = RealLy - elemY - 1;
				if (drawLx <= 0 || drawLy <= 0) continue;

				Xbar(elemX, elemY, drawLx, drawLy, (byte)254);

				if (BV->Nx > 1) {
					char ccc[16];
					snprintf(ccc, sizeof(ccc), "%d", BV->Choosed[index]);
					ShowString(elemX + 4, elemY + 4, ccc, &fn10);
				}
			}

			if (SSTAT && BV->Nx > 1) {
				char cc[32];
				snprintf(cc, sizeof(cc), "%d", index);
				int textY = y + (iy + 1) * BV->OneLy - 16;
				if (textY >= 0 && textY < RealLy) {
					ShowString(x + ix * BV->OneLx + 4, textY, cc, &fn10);
				}
			}
		}
	}
...
void CopyToScreen(int zx, int zy, int zLx, int zLy) {
	...
	byte* src = static_cast<byte*>(ScreenPtr) + x + y * SCRSizeX;
	byte* dst = static_cast<byte*>(RealScreenPtr) + x + y * RSCRSizeX;
...
void CopyToOffScreen(int zx, int zy, int srLx, int srLy, byte* data) {
	if (!bActive) return;
	...
	// Calculate source and destination pointers
	byte* src = data;
	byte* dst = static_cast<byte*>(ScreenPtr) + x + y * SCRSizeX;
...
void CopyToRealScreenMMX( int zx, int zy,
	int srLx, int srLy,
	byte* data );
void CopyToRealScreen(int zx, int zy, int srLx, int srLy, byte* data) {
	if (!bActive) return;
...
void CopyToRealScreenMMX(int zx, int zy, int srLx, int srLy, byte* data) {
	int x = zx;
	int y = zy;
	int Lx = srLx;
	int Ly = srLy;
...
void SQPicture::Draw(int x, int y) {
	if (!bActive || !this || !PicPtr) return;

	// Вычисляем размеры изображения
	int lx = static_cast<int>(PicPtr[0]);
	int ly = static_cast<int>(PicPtr[1]);

	// Проверка валидности размеров
	if (ly <= 0 || lx <= 0 || lx > RealLx || ly > RealLy) return;

	// Корректируем координаты и размеры, чтобы не выйти за границы экрана
	int draw_x = x;
	int draw_y = y;
	int draw_lx = lx;
	int draw_ly = ly;
	...
	if (draw_x < 0) {
		draw_lx += draw_x; // Уменьшаем ширину на отрицательное смещение
		draw_x = 0;
	}
	if (draw_y < 0) {
		draw_ly += draw_y; // Уменьшаем высоту на отрицательное смещение
		draw_y = 0;
	}
	if (draw_x + draw_lx > RealLx) {
		draw_lx = RealLx - draw_x; // Обрезаем по правой границе
	}
	if (draw_y + draw_ly > RealLy) {
		draw_ly = RealLy - draw_y; // Обрезаем по нижней границе
	}

	// Если после обрезки ничего не осталось для рисования, выходим
	if (draw_lx <= 0 || draw_ly <= 0) return;

	// Вычисляем начальные адреса
	byte* src = reinterpret_cast<byte*>(PicPtr + 2);
	byte* dst = static_cast<byte*>(ScreenPtr) + draw_x + draw_y * ScrWidth;

	// Корректируем src, если x < 0 (пропускаем пиксели, которые не рисуются)
	if (x < 0) {
		src -= x; // Сдвигаем src, чтобы начать с правильного пикселя
	}
	// Корректируем src для пропуска строк, если y < 0
	if (y < 0) {
		src -= y * lx; // Пропускаем строки, которые не рисуются
	}

	// Копируем данные построчно
	for (int i = 0; i < draw_ly; ++i) {
		std::memcpy(dst, src, draw_lx);
		src += lx; // Переходим к следующей строке изображения
		dst += ScrWidth; // Переходим к следующей строке экрана
	}
}
void SQPicture::DrawTransparent(int x, int y) {
	...
	// Вычисляем начальные адреса
	byte* src = reinterpret_cast<byte*>(PicPtr + 2);
	byte* dst = static_cast<byte*>(ScreenPtr) + draw_x + draw_y * SCRSizeX;
	...
	// Рисуем изображение с учётом прозрачности
	for (int i = 0; i < draw_ly; ++i) {
		byte* src_line = src;
		byte* dst_line = dst;
		for (int j = 0; j < draw_lx; ++j) {
			if (*src_line != 0) {
				*dst_line = *src_line;
			}
			++src_line;
		...
		src += lx; // Переходим к следующей строке изображения
		dst += SCRSizeX; // Переходим к следующей строке экрана
	}
}
bool SafeLoad = 0;
void SQPicture::LoadPicture(char* name)
{
	...
	if (ff1 != INVALID_HANDLE_VALUE)
	{
		...
		if (Lx > 0 && Ly > 0)
		{
			// Выделяем память для Lx и Ly (2 слова) + данных (Lx * Ly байт)
			PicPtr = (word*)(new char[2 * sizeof(word) + Lx * Ly]);
			if (!PicPtr)
			{
				RClose(ff1);
				return;
			}
			byte* data = reinterpret_cast<byte*>(PicPtr + 2); // Сдвигаем на 2 слова
			RBlockRead(ff1, data, Lx * Ly);
			PicPtr[0] = static_cast<word>(Lx);
			PicPtr[1] = static_cast<word>(Ly);

			// Заменяем 0xFF на 0xF6 в байтовых данных
...
void EncodeLine(byte* src, byte* dst, byte* scr, byte* tbl, byte* oddtbl) {
	// Обрабатываем пиксели парами
	for (int i = 0; i < TransLx / 2; ++i) {
		int idx = i * 2;
		// Чётный пиксель (используем tbl)
		int index = src[idx] + (dst[idx] << 8);
		scr[idx] = tbl[index];
		// Нечётный пиксель (используем oddtbl), если он есть
		if (idx + 1 < TransLx) {
			index = src[idx + 1] + (dst[idx + 1] << 8);
			scr[idx + 1] = oddtbl[index];
		}
	}
}
...
void EncodeLine2(byte* src, byte* dst, byte* scr, byte* tbl) {
	...
	for (int i = 0; i < TransLx / 2; ++i) {
		...
		scr[idx] = dst[idx];
		if (idx + 1 < TransLx) {
			int index = src[idx + 1] + (dst[idx + 1] << 8);
			scr[idx + 1] = tbl[index];
		}
	}
}
