...
{
	// Приведение param к указателю ADI_DBTBL
	ADI_DBTBL* TBL = (ADI_DBTBL*)param;

	// Проверка на nullptr для TBL
	if (!TBL || !TBL->TBL) {
		return; // Выходим, если TBL или TBL->TBL невалидны
	}

	// Проверка индекса Refs
	if (Index < 0 || Index >= TBL->TBL->NRefs || !TBL->TBL->Refs) {
		return; // Выходим, если Index вне диапазона или Refs невалиден
	}

	// Выбор палитры
	int gpid = CurPalette ? IBOR2 : IBOR0;

	// Отрисовка прямоугольника в зависимости от Active
	if (Active == 2) {
		DrawFilledRect3(x, y, x + Lx - 1, y + Ly - 1, (Corners*)PXA, gpid, 45, 3);
	}
	else if (Active == 1) {
		DrawFilledRect3(x, y, x + Lx - 1, y + Ly - 1, (Corners*)PXP, gpid, 45 + 11, 3);
	}

	// Количество колонок
	int nc = TBL->NColumns;
	if (nc <= 0 || !TBL->COLM) {
		return; // Выходим, если нет колонок или COLM невалиден
	}

	// Вычисление начального индекса строки
	int p0 = TBL->TBL->Refs[Index] * nc;
	if (p0 < 0) {
		return; // Выходим, если p0 отрицательный
	}

	// Высота строки для центровки текста
	int sy = GetRLCHeight(YellowFont.RLC, 'C');

	// Цикл по колонкам
	for (int i = 0; i < TBL->TBL->NCol && i < nc; i++) {
		// Проверка, что индекс в пределах массива Lines
		if (p0 + i >= TBL->TBL->NLines || !TBL->TBL->Lines) {
			continue; // Пропускаем, если индекс выходит за границы или Lines невалиден
		}

		// Получение строки
		char* s = TBL->TBL->Lines[p0 + i];
		if (!s) {
			continue; // Пропускаем, если строка невалидна
		}

		// Координата x для текущей колонки
		...
		if (vt == 4) {
			...
			int idx = atoi(s);
			if (idx >= 0) {
				char visparams_buf[512] = { 0 };
				char* visparams = TBL->COLM[i].VisParams;
				if (!visparams) {
					continue; // Пропускаем, если VisParams невалиден
				}

				int pos = 0;
				while (idx > 0 && visparams[pos]) {
					idx--;
				...
				// Отрисовка строки
				ShowPixString(TBL->SXP, x + 6 + xcl, y + ((Ly - sy) >> 1) - 1,
			...
			// Отрисовка строки для остальных типов
			ShowPixString(TBL->SXP, x + 6 + xcl, y + ((Ly - sy) >> 1) - 1,
				x + xcl + TBL->COLM[i].Width - 1, 256,
